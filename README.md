...
- Added required package metadata (`homepage`, `repository`, `bugs.url`) for Linux FPM/electron-builder `.deb` packaging validation.

- Linux package metadata is configured (`author.email` and `build.linux.maintainer`) so `.deb` builds do not fail maintainer validation.
- Build dependency upgraded to `electron-builder@^25.1.8` to pull in safer transitive dependency versions.
- Added `overrides.minimatch` set to `^10.2.1` to remediate vulnerable minimatch ranges in nested dependencies where possible.
- In-app auto-update checks/downloads are gated to packaged builds; development mode will return a clear message instead of AppImage-related runtime warnings.
